...
def test_build_encode_cmd_libx265():
    ...
    cmd = _build_encode_cmd(
        "input.mkv", "output.hevc", get_encoder("libx265"), meta, opts, video_only=True
    )
...
def test_build_encode_cmd_hevc_amf():
    ...
    cmd = _build_encode_cmd(
        "input.mkv",
        "output.hevc",
        get_encoder("hevc_amf"),
        meta,
        opts,
        video_only=True,
        source_bitrate=25000000,
    )
    idx = cmd.index("-c:v")
...
def test_build_encode_cmd_copy():
    ...
    cmd = _build_encode_cmd(
        "input.mkv", "output.hevc", get_encoder("copy"), meta, opts, video_only=True
    )
    idx = cmd.index("-c:v")
...
def test_build_encode_cmd_libsvtav1():
    ...
    cmd = _build_encode_cmd(
        "input.mkv", "output.ivf", get_encoder("libsvtav1"), meta, opts, video_only=True
    )
    idx = cmd.index("-c:v")
